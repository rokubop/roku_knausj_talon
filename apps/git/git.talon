...
git diff tool clip$:
    ...
    edit.paste()
    key(enter)
git diff tool head <number_small>: "git difftool HEAD~{number_small}\n"
git diff tool <user.cursorless_target>:
    text = user.cursorless_get_text(cursorless_target)
    user.vscode("workbench.action.terminal.focus")
    key("ctrl-c")
    insert("git difftool {text}^ {text}\n")
git diff name only <user.cursorless_target>:
    text = user.cursorless_get_text(cursorless_target)
    user.vscode("workbench.action.terminal.focus")
    key("ctrl-c")
    insert("git diff --name-only {text} | code -\n")
git diff$: "git diff\n"
